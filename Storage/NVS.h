...
auto NVS::StoreKeyValue(const std::string &key, T value, const std::string &fileName, bool overwrite) -> ErrorCode {
    ...
    if (!overwrite) {
        if (std::is_same<T, std::string>()) {
            ...
            err = nvs_get_u32(file_handle, key.c_str(), reinterpret_cast<uint32_t *>(&integerValue));
        } else if (std::is_same<T, std::int64_t>()) {
            err = nvs_get_i64(file_handle, key.c_str(), reinterpret_cast<int64_t *>(&integerValue));
        } else if (std::is_same<T, std::uint64_t>()) {
            err = nvs_get_u64(file_handle, key.c_str(), reinterpret_cast<uint64_t *>(&integerValue));
        } else {
            ESP_LOGE(__FUNCTION__, "Tipo invalido ou nao suportado");
        }
        if (err == ESP_OK) {
...
#endif
    if (std::is_same<T, std::string>()) {
        err = nvs_set_str(file_handle, key.c_str(), reinterpret_cast<std::string *>(&value)->c_str());
    } else if (std::is_same<T, std::int32_t>()) {

...
/* STATIC */ BLEClient *BLEDevice::createClient() {
    ESP_LOGV(__FUNCTION__, ">> createClient");
#ifndef CONFIG_GATTC_ENABLE  // Check that BLE GATTC is enabled in make menuconfig
    ESP_LOGE(__FUNCTION__, "BLE GATTC is not enabled - CONFIG_GATTC_ENABLE not defined");
    abort();
...
/* STATIC */ BLEServer *BLEDevice::createServer() {
    ESP_LOGV(__FUNCTION__, ">> createServer");
#ifndef CONFIG_GATTS_ENABLE  // Check that BLE GATTS is enabled in make menuconfig
    ESP_LOGE(__FUNCTION__,"BLE GATTS is not enabled - CONFIG_GATTS_ENABLE not defined");
    abort();
...
/* STATIC */ void BLEDevice::gattServerEventHandler(
    ...
    ESP_LOGD(__FUNCTION__, "gattServerEventHandler [esp_gatt_if: %d] ... %s",
             gatts_if,
...
/* STATIC */ void BLEDevice::gattClientEventHandler(
    ...
    ESP_LOGD(__FUNCTION__, "gattClientEventHandler [esp_gatt_if: %d] ... %s",
    ...
    for (auto &myPair : BLEDevice::getPeerDevices(true)) {
        ...
        if (((BLEClient *) conn_status.peer_device)->getGattcIf() == gattc_if ||
            ((BLEClient *) conn_status.peer_device)->getGattcIf() == ESP_GATT_IF_NONE || gattc_if == ESP_GATT_IF_NONE) {
...
/* STATIC */ void BLEDevice::gapEventHandler(
    ...
    switch (event) {
        ...
        case ESP_GAP_BLE_NC_REQ_EVT:                                /*  NUMERIC CONFIRMATION  */
            ESP_LOGI(__FUNCTION__, "ESP_GAP_BLE_NC_REQ_EVT");
#ifdef CONFIG_BLE_SMP_ENABLE   // Check that BLE SMP (security) is configured in make menuconfig
        ...
        case ESP_GAP_BLE_SEC_REQ_EVT:
            ...
            ESP_LOGI(__FUNCTION__, "ESP_GAP_BLE_SEC_REQ_EVT");
#ifdef CONFIG_BLE_SMP_ENABLE   // Check that BLE SMP (security) is configured in make menuconfig
        if(BLEDevice::m_securityCallbacks!=nullptr){
        ...
        case ESP_GAP_BLE_PASSKEY_NOTIF_EVT:  //the app will receive this evt when the IO  has Output capability and the peer device IO has Input capability.
            ...
            ESP_LOGI(__FUNCTION__, "ESP_GAP_BLE_PASSKEY_NOTIF_EVT");
#ifdef CONFIG_BLE_SMP_ENABLE   // Check that BLE SMP (security) is configured in make menuconfig
        ESP_LOGI(__FUNCTION__,"passKey = %d", param->ble_security.key_notif.passkey);
        if(BLEDevice::m_securityCallbacks!=nullptr){
        ...
        case ESP_GAP_BLE_KEY_EVT:
            ...
            ESP_LOGD(__FUNCTION__, "ESP_GAP_BLE_KEY_EVT");
#ifdef CONFIG_BLE_SMP_ENABLE   // Check that BLE SMP (security) is configured in make menuconfig
            ESP_LOGI(__FUNCTION__,"key type = %s", BLESecurity::esp_key_type_to_str(param->ble_security.ble_key.key_type));
#endif    // CONFIG_BLE_SMP_ENABLE
            break;
        case ESP_GAP_BLE_AUTH_CMPL_EVT:
            ESP_LOGI(__FUNCTION__, "ESP_GAP_BLE_AUTH_CMPL_EVT");
#ifdef CONFIG_BLE_SMP_ENABLE   // Check that BLE SMP (security) is configured in make menuconfig
        if(BLEDevice::m_securityCallbacks != nullptr){
...
/* STATIC */ BLEScan *BLEDevice::getScan() {
    //ESP_LOGV(__FUNCTION__,">> getScan");
    if (m_pScan == nullptr) {
        m_pScan = new BLEScan();
        //ESP_LOGD(__FUNCTION__," - creating a new scan object");
    }
    //ESP_LOGV(__FUNCTION__,"<< getScan: Returning object at 0x%x", (uint32_t)m_pScan);
    return m_pScan;
} // getScan
...
/* STATIC */ std::string BLEDevice::getValue(BLEAddress bdAddress, BLEUUID serviceUUID, BLEUUID characteristicUUID) {
    ESP_LOGV(__FUNCTION__, ">> getValue: bdAddress: %s, serviceUUID: %s, characteristicUUID: %s",
             bdAddress.toString().c_str(),
    ...
    pClient->disconnect();
    ESP_LOGV(__FUNCTION__, "<< getValue");
    return ret;
} // getValue
...
/* STATIC */ void BLEDevice::init(std::string deviceName) {
    if (!initialized) {
        ...
        if (errRc != ESP_OK) {
            ESP_LOGE(__FUNCTION__, "nvs_flash_init: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
            return;
        }
        ...
        if (errRc != ESP_OK) {
            ESP_LOGE(__FUNCTION__, "esp_bt_controller_init: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
            return;
        }
        ...
        if (errRc != ESP_OK) {
            ESP_LOGE(__FUNCTION__, "esp_bt_controller_enable: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
            return;
        }
#else
        ...
        if (bt_state == ESP_BLUEDROID_STATUS_UNINITIALIZED) {
            ...
            if (errRc != ESP_OK) {
                ESP_LOGE(__FUNCTION__, "esp_bluedroid_init: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
                return;
            }
        }
        ...
        if (bt_state != ESP_BLUEDROID_STATUS_ENABLED) {
            ...
            if (errRc != ESP_OK) {
                ESP_LOGE(__FUNCTION__, "esp_bluedroid_enable: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
                return;
            }
        }
        ...
        if (errRc != ESP_OK) {
            ESP_LOGE(__FUNCTION__, "esp_ble_gap_register_callback: rc=%d %s", errRc,
        ...
        if (errRc != ESP_OK) {
            ESP_LOGE(__FUNCTION__,"esp_ble_gattc_register_callback: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
            return;
        }
#endif   // CONFIG_GATTC_ENABLE
        ...
        if (errRc != ESP_OK) {
            ESP_LOGE(__FUNCTION__, "esp_ble_gatts_register_callback: rc=%d %s", errRc,
                     GeneralUtils::errorToString(errRc));
            return;
        }
#endif   // CONFIG_GATTS_ENABLE
        ...
        if (errRc != ESP_OK) {
            ESP_LOGE(__FUNCTION__, "esp_ble_gap_set_device_name: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
            return;
        };
        ...
        if (errRc != ESP_OK) {
            ESP_LOGE(__FUNCTION__,"esp_ble_gap_set_security_param: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
            return;
        };
#endif // CONFIG_BLE_SMP_ENABLE
    }
    vTaskDelay(
            200 / portTICK_PERIOD_MS); // Delay for 200 msecs as a workaround to an apparent Arduino environment issue.
} // init
...
/* STATIC */ void BLEDevice::setPower(esp_power_level_t powerLevel, esp_ble_power_type_t powerType) {
    ESP_LOGV(__FUNCTION__, ">> setPower: %d (type: %d)", powerLevel, powerType);
    esp_err_t errRc = ::esp_ble_tx_power_set(powerType, powerLevel);
    if (errRc != ESP_OK) {
        ESP_LOGE(__FUNCTION__, "esp_ble_tx_power_set: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
    };
    ESP_LOGV(__FUNCTION__, "<< setPower");
} // setPower
 ...
 */
/* STATIC */ void
BLEDevice::setValue(BLEAddress bdAddress, BLEUUID serviceUUID, BLEUUID characteristicUUID, std::string value) {
    ESP_LOGV(__FUNCTION__, ">> setValue: bdAddress: %s, serviceUUID: %s, characteristicUUID: %s",
             bdAddress.toString().c_str(),
             serviceUUID.toString().c_str(), characteristicUUID.toString().c_str());
    BLEClient *pClient = createClient();
    pClient->connect(bdAddress);
...
void BLEDevice::whiteListAdd(BLEAddress address) {
    ESP_LOGV(__FUNCTION__, ">> whiteListAdd: %s", address.toString().c_str());
    esp_err_t errRc = esp_ble_gap_update_whitelist(true, *address.getNative(),
                                                   BLE_WL_ADDR_TYPE_PUBLIC);  // True to add an entry.
    if (errRc != ESP_OK) {
        ESP_LOGE(__FUNCTION__, "esp_ble_gap_update_whitelist: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
    }
    ESP_LOGV(__FUNCTION__, "<< whiteListAdd");
} // whiteListAdd
...
void BLEDevice::whiteListRemove(BLEAddress address) {
    ESP_LOGV(__FUNCTION__, ">> whiteListRemove: %s", address.toString().c_str());
    esp_err_t errRc = esp_ble_gap_update_whitelist(false, *address.getNative(),
                                                   BLE_WL_ADDR_TYPE_PUBLIC);  // False to remove an entry.
    if (errRc != ESP_OK) {
        ESP_LOGE(__FUNCTION__, "esp_ble_gap_update_whitelist: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
    }
    ESP_LOGV(__FUNCTION__, "<< whiteListRemove");
} // whiteListRemove
...
esp_err_t BLEDevice::setMTU(uint16_t mtu) {
    ESP_LOGV(__FUNCTION__, ">> setLocalMTU: %d", mtu);
    esp_err_t err = esp_ble_gatt_set_local_mtu(mtu);
    if (err == ESP_OK) {
        m_localMTU = mtu;
    } else {
        ESP_LOGE(__FUNCTION__, "can't set local mtu value: %d", mtu);
    }
    ESP_LOGV(__FUNCTION__, "<< setLocalMTU");
    return err;
}
...
BLEAdvertising *BLEDevice::getAdvertising() {
    if (m_bleAdvertising == nullptr) {
        ...
        ESP_LOGI(__FUNCTION__, "create advertising");
    }
    ESP_LOGD(__FUNCTION__, "get advertising");
    return m_bleAdvertising;
}

void BLEDevice::startAdvertising() {
    ESP_LOGV(__FUNCTION__, ">> startAdvertising");
    getAdvertising()->start();
    ESP_LOGV(__FUNCTION__, "<< startAdvertising");
} // startAdvertising

void BLEDevice::stopAdvertising() {
    ESP_LOGV(__FUNCTION__, ">> stopAdvertising");
    getAdvertising()->stop();
    ESP_LOGV(__FUNCTION__, "<< stopAdvertising");
} // stopAdvertising
...
void BLEDevice::updatePeerDevice(void *peer, bool _client, uint16_t conn_id) {
    ESP_LOGD(__FUNCTION__, "update conn_id: %d, GATT role: %s", conn_id, _client ? "client" : "server");
...
void BLEDevice::addPeerDevice(void *peer, bool _client, uint16_t conn_id) {
    ESP_LOGI(__FUNCTION__, "add conn_id: %d, GATT role: %s", conn_id, _client ? "client" : "server");
...
void BLEDevice::removePeerDevice(uint16_t conn_id, bool _client) {
    ESP_LOGI(__FUNCTION__, "remove: %d, GATT role %s", conn_id, _client ? "client" : "server");

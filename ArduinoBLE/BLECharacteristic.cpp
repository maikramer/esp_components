 ...
 */
BLECharacteristic::BLECharacteristic(const char *uuid, uint32_t properties) : BLECharacteristic(BLEUUID(uuid),
                                                                                                properties) {
}
...
void BLECharacteristic::addDescriptor(BLEDescriptor *pDescriptor) {
    ESP_LOGV(__FUNCTION__, ">> addDescriptor(): Adding %s to %s", pDescriptor->toString().c_str(), toString().c_str());
    m_descriptorMap.setByUUID(pDescriptor->getUUID(), pDescriptor);
    ESP_LOGV(__FUNCTION__, "<< addDescriptor()");
} // addDescriptor
...
void BLECharacteristic::executeCreate(BLEService *pService) {
    ESP_LOGV(__FUNCTION__, ">> executeCreate()");

    if (m_handle != NULL_HANDLE) {
        ESP_LOGE(__FUNCTION__, "Characteristic already has a handle.");
        return;
    }

    m_pService = pService; // Save the service to which this characteristic belongs.

    ESP_LOGD(__FUNCTION__, "Registering characteristic (esp_ble_gatts_add_char): uuid: %s, service: %s",
    ...
    if (errRc != ESP_OK) {
        ESP_LOGE(__FUNCTION__, "<< esp_ble_gatts_add_char: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
        return;
    }
...
void BLECharacteristic::handleGATTServerEvent(
    ...
    ESP_LOGV(__FUNCTION__, ">> handleGATTServerEvent: %s", BLEUtils::gattServerEventTypeToString(event).c_str());
...
// ???
            ...
            if (errRc != ESP_OK) {
                ESP_LOGE(__FUNCTION__, "esp_ble_gatts_send_response: rc=%d %s", errRc,
...
// to send a response.  If we do, then we formulate a response and send it.
            if (param->write.handle == m_handle) {
                ...
                ESP_LOGD(__FUNCTION__, " - Response to write event: New value: handle: %.2x, uuid: %s",
                         getHandle(), getUUID().toString().c_str());

                char *pHexData = BLEUtils::buildHexData(nullptr, param->write.value, param->write.len);
                ESP_LOGD(__FUNCTION__, " - Data: length: %d, data: %s", param->write.len, pHexData);
                free(pHexData);
...
//
                ...
                ESP_LOGD(__FUNCTION__, "mtu value: %d", maxOffset);
                if (param->read.need_rsp) {
                    ESP_LOGD(__FUNCTION__, "Sending a response (esp_ble_gatts_send_response)");
                    esp_gatt_rsp_t rsp;
                    ...
                    ESP_LOGD(__FUNCTION__, " - Data: length=%d, data=%s, offset=%d", rsp.attr_value.len, pHexData,
                             rsp.attr_value.offset);
                    free(pHexData);
        ...
        case ESP_GATTS_CONF_EVT: {
            // ESP_LOGD(__FUNCTION__,"m_handle = %d, conf->handle = %d", m_handle, param->conf.handle);
    ...
    ESP_LOGV(__FUNCTION__, "<< handleGATTServerEvent");
...
void BLECharacteristic::indicate() {
    ...
    ESP_LOGV(__FUNCTION__, ">> indicate: length: %d", m_value.getValue().length());
    notify(false);
    ESP_LOGV(__FUNCTION__, "<< indicate");
} // indicate
...
void BLECharacteristic::notify(bool is_notification) {
    ESP_LOGV(__FUNCTION__, ">> notify: length: %d", m_value.getValue().length());
    ...
    if (getService()->getServer()->getConnectedCount() == 0) {
        ESP_LOGV(__FUNCTION__, "<< notify: No connected clients.");
    ...
    auto *p2902 = (BLE2902 *) getDescriptorByUUID((uint16_t) 0x2902);
    if (is_notification) {
        if (p2902 != nullptr && !p2902->getNotifications()) {
            ESP_LOGV(__FUNCTION__, "<< notifications disabled; ignoring");
            m_pCallbacks->onStatus(this, BLECharacteristicCallbacks::Status::ERROR_NOTIFY_DISABLED,
                                   0);   // Invoke the notify callback.
            return;
        }
    } else {
        if (p2902 != nullptr && !p2902->getIndications()) {
            ESP_LOGV(__FUNCTION__, "<< indications disabled; ignoring");
            m_pCallbacks->onStatus(this, BLECharacteristicCallbacks::Status::ERROR_INDICATE_DISABLED,
                                   0);   // Invoke the notify callback.
            return;
        }
    }
    for (auto &myPair : getService()->getServer()->getPeerDevices(false)) {
        ...
        if (m_value.getValue().length() > _mtu - 3) {
            ESP_LOGW(__FUNCTION__, "- Truncating to %d bytes (maximum notify size)", _mtu - 3);
        }
        ...
        esp_err_t errRc = ::esp_ble_gatts_send_indicate(
                ...
                myPair.first,
                getHandle(), length, (uint8_t *) m_value.getValue().data(),
                !is_notification); // The need_confirm = false makes this a notify.
        if (errRc != ESP_OK) {
            ESP_LOGE(__FUNCTION__, "<< esp_ble_gatts_send_ %s: rc=%d %s", is_notification ? "notify" : "indicate",
                     errRc,
                     GeneralUtils::errorToString(errRc));
            m_semaphoreConfEvt.give();
            m_pCallbacks->onStatus(this, BLECharacteristicCallbacks::Status::ERROR_GATT,
                                   errRc);   // Invoke the notify callback.
            return;
        }
        if (!is_notification) { // is indication
            if (!m_semaphoreConfEvt.timedWait("indicate", indicationTimeout)) {
                m_pCallbacks->onStatus(this, BLECharacteristicCallbacks::Status::ERROR_INDICATE_TIMEOUT,
                                       0);   // Invoke the notify callback.
            } else {
                auto code = (esp_gatt_status_t) m_semaphoreConfEvt.value();
                if (code == ESP_GATT_OK) {
                    m_pCallbacks->onStatus(this, BLECharacteristicCallbacks::Status::SUCCESS_INDICATE,
                                           code);   // Invoke the notify callback.
                } else {
                    m_pCallbacks->onStatus(this, BLECharacteristicCallbacks::Status::ERROR_INDICATE_FAILURE, code);
                }
            }
        } else {
            m_pCallbacks->onStatus(this, BLECharacteristicCallbacks::Status::SUCCESS_NOTIFY,
                                   0);   // Invoke the notify callback.
        }
    }
    ESP_LOGV(__FUNCTION__, "<< notify");
} // Notify
...
void BLECharacteristic::setBroadcastProperty(bool value) {
    //ESP_LOGD(__FUNCTION__,"setBroadcastProperty(%d)", value);
    if (value) {
...
//    ESP_LOGV(__FUNCTION__,">> setCallbacks: 0x%x", (uint32_t) pCallbacks);
    if (pCallbacks != nullptr) {
        m_pCallbacks = pCallbacks;
    } else {
        m_pCallbacks = &defaultCallback;
    }
    ESP_LOGV(__FUNCTION__, "<< setCallbacks");
} // setCallbacks
...
void BLECharacteristic::setHandle(uint16_t handle) {
    ESP_LOGV(__FUNCTION__, ">> setHandle: handle=0x%.2x, characteristic uuid=%s", handle, getUUID().toString().c_str());
    m_handle = handle;
    ESP_LOGV(__FUNCTION__, "<< setHandle");
} // setHandle
...
void BLECharacteristic::setIndicateProperty(bool value) {
    //ESP_LOGD(__FUNCTION__,"setIndicateProperty(%d)", value);
    if (value) {
...
void BLECharacteristic::setNotifyProperty(bool value) {
    //ESP_LOGD(__FUNCTION__,"setNotifyProperty(%d)", value);
    if (value) {
...
void BLECharacteristic::setReadProperty(bool value) {
    //ESP_LOGD(__FUNCTION__,"setReadProperty(%d)", value);
    if (value) {
...
void BLECharacteristic::setValue(uint8_t *data, size_t length) {
    ...
    ESP_LOGV(__FUNCTION__, ">> setValue: length=%d, data=%s, characteristic UUID=%s", length, pHex,
             getUUID().toString().c_str());
    free(pHex);
    if (length > ESP_GATT_MAX_ATTR_LEN) {
        ESP_LOGE(__FUNCTION__, "Size %d too large, must be no bigger than %d", length, ESP_GATT_MAX_ATTR_LEN);
        return;
    }
    m_semaphoreSetValue.take();
    m_value.setValue(data, length);
    m_semaphoreSetValue.give();
    ESP_LOGV(__FUNCTION__, "<< setValue");
} // setValue
...
void BLECharacteristic::setWriteNoResponseProperty(bool value) {
    //ESP_LOGD(__FUNCTION__,"setWriteNoResponseProperty(%d)", value);
    if (value) {
...
void BLECharacteristic::setWriteProperty(bool value) {
    //ESP_LOGD(__FUNCTION__,"setWriteProperty(%d)", value);
    if (value) {
...
void BLECharacteristicCallbacks::onNotify(BLECharacteristic *pCharacteristic) {
    ESP_LOGD("BLECharacteristicCallbacks", ">> onNotify: default");
    ESP_LOGD("BLECharacteristicCallbacks", "<< onNotify");
} // onNotify
...
void BLECharacteristicCallbacks::onStatus(BLECharacteristic *pCharacteristic, Status s, uint32_t code) {
    ESP_LOGD("BLECharacteristicCallbacks", ">> onStatus: default");
    ESP_LOGD("BLECharacteristicCallbacks", "<< onStatus");
} // onStatus

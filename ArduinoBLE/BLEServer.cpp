...
BLEService *BLEServer::createService(BLEUUID uuid, uint32_t numHandles, uint8_t inst_id) {
    ESP_LOGV(__FUNCTION__, ">> createService - %s", uuid.toString().c_str());
    m_semaphoreCreateEvt.take("createService");

    // Check that a service with the supplied UUID does not already exist.
    if (m_serviceMap.getByUUID(uuid) != nullptr) {
        ESP_LOGW(__FUNCTION__,
                 "<< Attempt to create a new service with uuid %s but a service with that UUID already exists.",
                 uuid.toString().c_str());
    }
    ...
    ESP_LOGV(__FUNCTION__, "<< createService");
    return pService;
} // createService
 ...
 */
void
BLEServer::handleGATTServerEvent(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ESP_LOGV(__FUNCTION__, ">> handleGATTServerEvent: %s",
    ...
    switch (event) {
        ...
        case ESP_GATTS_CREATE_EVT: {
            BLEService *pService = m_serviceMap.getByUUID(param->create.service_id.id.uuid,
                                                          param->create.service_id.id.inst_id);  // <--- very big bug for multi services with the same uuid
...
void BLEServer::registerApp(uint16_t m_appId) {
    ESP_LOGV(__FUNCTION__, ">> registerApp - %d", m_appId);
    ...
    ESP_LOGV(__FUNCTION__, "<< registerApp");
} // registerApp
...
bool BLEServer::connect(BLEAddress address) {
    ...
    if (errRc != ESP_OK) {
        ESP_LOGE(__FUNCTION__, "esp_ble_gattc_open: rc=%d %s", errRc, GeneralUtils::errorToString(errRc));
        return false;
    }

    uint32_t rc = m_semaphoreOpenEvt.wait("connect");   // Wait for the connection to complete.
    ESP_LOGV(__FUNCTION__, "<< connect(), rc=%d", rc == ESP_GATT_OK);
 ...
 */
void BLEServer::updateConnParams(esp_bd_addr_t remote_bda, uint16_t minInterval, uint16_t maxInterval, uint16_t latency,
                                 uint16_t timeout) {
